...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        String processUri = request.getUri();
        File file = new File(root, request.getUri());
        if (file.isDirectory()) {
            processUri = processUri + "/index.html";
            file = new File(root, processUri);
        }
        ...
        if (file.isFile()) {
            ...
            try {
                ...
                eTag = String.format("%032X", new BigInteger(1, hash));
            } catch (NoSuchAlgorithmException e) {
                System.err.println("You lost: Error during hashing");
            }
    ...
    private static final class Request {
        ...
        private Request(String request) {
            ...
            try {
                ...
                System.err.println("You lost: UTF-8 doesn't support");
            }
            if (firstLineTokens[1].startsWith("/")) {
                firstLineTokens[1] = firstLineTokens[1].substring(1);
            }
