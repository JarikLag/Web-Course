...
public class ArticleRepositoryImpl implements ArticleRepository {
    ...
    public List<Article> findByUser(long userId) {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            ...
            throw new RepositoryException("Can't find all articles by user.", e);
        }
        return articles;
    }
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                ...
                if (statement.executeUpdate() == 1) {
                    ...
                    throw new RepositoryException("Updated more than one Article.");
                }
            }
        } catch (SQLException e) {
            throw new RepositoryException("Can't update Article.hidden.", e);
        }
    }
