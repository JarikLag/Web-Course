...
public class UserRepositoryImpl implements UserRepository {
    ...
    public User findByLoginOrEmailAndPasswordSha(String loginOrEmail, String passwordSha) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            ...
            throw new RepositoryException("Can't find User by login and passwordSha.", e);
        }
    }
    ...
    public void save(User user, String passwordSha) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    "INSERT INTO User (login, passwordSha, email, confirmed, creationTime) VALUES (?, ?, ?, ?, NOW())",
                    Statement.RETURN_GENERATED_KEYS)) {
                statement.setString(1, user.getLogin());
                statement.setString(2, passwordSha);
                statement.setString(3, user.getEmail());
                statement.setBoolean(4, user.getConfirmed());
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                ...
                if (statement.executeUpdate() == 1) {
                    ...
                    throw new RepositoryException("Updated more than one User.");
                }
            }
        } catch (SQLException e) {
            throw new RepositoryException("Can't update User.confirmed.", e);
        }
    }
