...
public class FrontServlet extends HttpServlet {
    ...
    private void processRoute(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        String method = "";
        try {
            for (String m : Arrays.asList("before", route.getAction(), "after")) {
                method = m;
                callPageMethod(page, method, request, view);
            }
    ...
    private void callPageMethod(Object page, String methodName, HttpServletRequest request, Map<String, Object> view)
            throws NotFoundException, InvocationTargetException, IllegalAccessException {
        Method method = null;
        for (Class<?> c = page.getClass(); method == null && c != null; c = c.getSuperclass()) {
            method = Arrays.stream(c.getDeclaredMethods())
        ...
        method.invoke(page, invokeArgs.toArray());
    }
